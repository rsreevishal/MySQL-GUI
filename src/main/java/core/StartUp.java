...
public class StartUp {
	public static void call() {
		...
		try {
			...
			st.executeUpdate();
			st = sqlConnection.prepareStatement("CREATE TABLE IF NOT EXISTS mysqlgui_table_fields(table_id INT, fieldname VARCHAR(50), fieldtype VARCHAR(50), constraints VARCHAR(50), FOREIGN KEY (table_id) REFERENCES mysqlgui_tables(id));");
			st.executeUpdate();
